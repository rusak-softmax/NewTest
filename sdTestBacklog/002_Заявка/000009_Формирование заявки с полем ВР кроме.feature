...
Сценарий: Формирование заявки с обязательными полями КРОМЕ поля Вид работ
	...
	И     таблица "УстановленноеОборудование" содержит строки:
		...
		| '*' | 'Оборудование 3 Объекта 1' |
	# *
	И я нажимаю кнопку очистить у поля с именем "ВидРабот"
	# обязательное поле: Должен сработать запрет создания ЗАЯВКИ без заполненного обязательного поля Вид Работ
	# Сегмент с контрольными точками:
	# *
	И     элемент формы с именем "Абонент" стал равен 'Потребитель 1'
	И     элемент формы с именем "ЛицевойСчет" стал равен '000111'
