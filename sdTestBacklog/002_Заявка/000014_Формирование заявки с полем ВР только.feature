...
Сценарий: Формирование заявки с обязательными полями ТОЛЬКО поле Вид работ
	...
	# так как проверка только Вид работ
	# Сегмент с контрольными точками:
	# *
	И     элемент формы с именем "Абонент" стал равен ''
	И     элемент формы с именем "ЛицевойСчет" стал равен ''
	...
	# Предложение: Возможно, было бы правильней выдавать информационные сообщение обо всех незаполненных обязательных реквизитах, т.е. в т.ч. и Объект обслуживания, Адресат, Сроки исполнения
	И Я закрываю текущее окно
	# информационное сообщение
	Тогда открылось окно '1С:Предприятие' 
