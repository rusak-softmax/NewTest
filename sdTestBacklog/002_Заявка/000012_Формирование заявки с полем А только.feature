...
Сценарий: Формирование заявки с обязательными полями ТОЛЬКО поле Адресат
	...
	# так как проверка только Адресат
	# Сегмент с контрольными точками:
	# *
	И     элемент формы с именем "Абонент" стал равен ''
	И     элемент формы с именем "ЛицевойСчет" стал равен ''
	...
	# Сегмент с контрольными точками:
	# *
	Тогда таблица "ГруппыПользователей" стала равной:
		| 'Наименование'     |
		| 'Все пользователи' |
	Тогда в таблице "ПользователиСписок" количество строк "равно" 0
	# *
	И я нажимаю на кнопку с именем 'ВыбратьПользователя'
	# окно не закрылось, так как нечего выбирать
	...
	# Предложение: Возможно, было бы правильней выдавать информационные сообщение обо всех незаполненных обязательных реквизитах, т.е. в т.ч. и Объект обслуживания, Вид работ, Адресат, Срок исполнения
	И Я закрываю текущее окно
	# информационное сообщение
	Тогда открылось окно '1С:Предприятие' 
